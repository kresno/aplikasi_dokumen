...
class Datatables
  {
    ...
    */
	...
	/**
    * Generates the column for search
    *
    * @param string $columns
    * @param bool $backtick_protect
    * @return mixed
    */
    public function search_column($columns, $backtick_protect = TRUE)
    {
      foreach($this->explode(',', $columns) as $val)
      {
        $this->search_columns[] =  trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$1', $val));
      }
      return $this;
    }
    /**
	  ...
	  if ($this->search_columns){	  
		  ...
		  if($sSearch != '')
			for($i = 0; $i < count($this->search_columns); $i++)
				$sWhere .= $this->search_columns[$i] . " LIKE '%" . $sSearch . "%' OR ";
		  $sWhere = substr_replace($sWhere, '', -3);
		  if($sWhere != '')
			$this->ci->db->where('(' . $sWhere . ')');
		  // TODO : sRangeSeparator
    ...
    */
    private function produce_output($output, $charset)
    {
      ...
      if($output == 'json')
      {
        ...
        if($charset == 'utf-8')
          return json_encode($sOutput, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
        else
