...
class M_transact extends CI_Model
{
  ...
  public function updateCommentFilePD($data)
  {
    $this->db->set('catatan_pd', $data['catatan'])
             ->set('file', $data['file'])
             ->where('id', $data['t_id']) 
  ...
  {
    $this->db->select('a.id, a.nama as nama, c.nama as jenis_dok, a.file as file, b.nama as opd, a.status as status, a.catatan_bidang as catatan_bidang,a.catatan_pd as catatan_pd');
    $this->db->from('transact a');
    $this->db->join('opd b', 'b.id=a.opd_id');
    $this->db->join('document c', 'c.id=a.doc_id');
    $query = $this->db->get();
